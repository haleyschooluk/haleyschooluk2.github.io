...
var adsIsQueued = false, rewardedIsReady = false, rewardedIsLoading = false, c2CB = {onSuccess:[], onFail:[]}, C2cbOnVideoDidAppear="", C2cbOnVideoDidDisappear ="";
...
function preloadReward(){
	if(rewardedIsReady || rewardedIsLoading || typeof gdsdk === "undefined" || typeof gdsdk.preloadAd === "undefined") return;
...
function launchTJEvent(eventName, C2cbOnFail, C2cbOnVideoDidAppear, C2cbOnVideoDidDisappear, forceReload, fallBackOnOfferWall){
	if(isTJEventReady(eventName) === "0" ){
		c2_callFunction(C2cbOnFail, ["0"]);
		c2_callFunction(C2cbOnVideoDidDisappear, ["0"]);
		return;
	}
	if(typeof gdsdk !== "undefined" && typeof gdsdk.showAd !== "undefined") gdsdk.showAd('rewarded');
	...
	setTimeout(function(){	c2_callFunction(C2cbOnVideoDidAppear, ["1"]);	}, 100);
	setTimeout(function(){	c2_callFunction(C2cbOnVideoDidDisappear, ["1"]);	}, 1000);
	setTimeout(function(){	preloadReward();	}, 30 * 1000);
}
